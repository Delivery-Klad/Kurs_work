...
def main():
    check = True
    while check:  # пока условие является правдой,то выполняются следущте действия(на экран выводится текст,...
        print("\n[1] - Посмотреть список книг"
              "\n[2] - Добавить новую книгу"
              "\n[3] - Удалить книгу"
              "\n[4] - Сортировка книг по году издания"
              "\n[5] - Сформированить список авторов"
              "\n[6] - Завершить работу")  # Вот эти действия
        inp = int(input("Выберите действие: "))  # Вводим номер операции
        if inp == 1:  # Вводим номер операции = 1 и так далее
            print_lib()
        if inp == 2:
